...
export const createArticulo = async (req: Request, res: Response) => {
  try {
    ...
    // Verificar código único por empresa
    const existingArticulo = await AppDataSource.createQueryBuilder()
      .select('1')
      .from(tableName, 'articulo')
    ...
    if (req.body.codigo && !insertData.ccod) insertData.ccod = req.body.codigo;
    if (req.body.descripcion && !insertData.cdet) insertData.cdet = req.body.descripcion;
    if (req.body.precio !== undefined && insertData.npre1 === undefined) insertData.npre1 = req.body.precio;
    if (req.body.stock !== undefined && insertData.ncan1 === undefined) insertData.ncan1 = req.body.stock;
    if (req.body.cantidad !== undefined && insertData.ncan1 === undefined) insertData.ncan1 = req.body.cantidad;
    if (req.body.marca && !insertData.cmar) insertData.cmar = req.body.marca;

    // CamelCase to SnakeCase helpers
    if (req.body.isDeleted !== undefined && insertData.is_deleted === undefined) insertData.is_deleted = req.body.isDeleted;
    if (req.body.lastSyncedAt !== undefined && insertData.last_synced_at === undefined) insertData.last_synced_at = req.body.lastSyncedAt;
    if (req.body.deviceId !== undefined && insertData.device_id === undefined) insertData.device_id = req.body.deviceId;
    if (req.body.fecha !== undefined && insertData.dfec === undefined) insertData.dfec = req.body.fecha;
...
export const updateArticulo = async (req: Request, res: Response) => {
  try {
    ...
    // Map frontend aliases/camelCase to DB columns
    if (req.body.codigo && !updateData.ccod) updateData.ccod = req.body.codigo;
    if (req.body.descripcion && !updateData.cdet) updateData.cdet = req.body.descripcion;
    if (req.body.precio !== undefined && updateData.npre1 === undefined) updateData.npre1 = req.body.precio;
    if (req.body.stock !== undefined && updateData.ncan1 === undefined) updateData.ncan1 = req.body.stock;
    if (req.body.cantidad !== undefined && updateData.ncan1 === undefined) updateData.ncan1 = req.body.cantidad;
    if (req.body.marca && !updateData.cmar) updateData.cmar = req.body.marca;

    // CamelCase to SnakeCase helpers
    if (req.body.isDeleted !== undefined && updateData.is_deleted === undefined) updateData.is_deleted = req.body.isDeleted;
    if (req.body.lastSyncedAt !== undefined && updateData.last_synced_at === undefined) updateData.last_synced_at = req.body.lastSyncedAt;
    if (req.body.deviceId !== undefined && updateData.device_id === undefined) updateData.device_id = req.body.deviceId;
    if (req.body.fecha !== undefined && updateData.dfec === undefined) updateData.dfec = req.body.fecha;
    ...
    const articuloActualizado = await AppDataSource.createQueryBuilder()
      .select('*')
      .from(tableName, 'articulo')
      .where('articulo.xxx = :id', { id: Number(id) })
      .getRawOne();
